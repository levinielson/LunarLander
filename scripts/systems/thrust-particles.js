MyGame.systems.ThrustParticles = function(spec) {
  'use strict';
  ...
  function create() {
      ...
      let p = {
              center: { 
                x: MyGame.ship.ship.center.x - Math.sin(MyGame.ship.ship.rotation) * Math.sqrt(MyGame.ship.ship.size),
                y: MyGame.ship.ship.center.y + Math.cos(MyGame.ship.ship.rotation) * Math.sqrt(MyGame.ship.ship.size) },
              size: { x: size, y: size},  // Making square particles
              direction: Random.nextCircleArc(MyGame.ship.ship.rotation + Math.PI),
